#include "pch.h"
#include "Interface/Fonts/Embedded/EmbeddedFontMain.h"
#include "Interface/Fonts/Embedded/EmbeddedFontFallback.h"
...
namespace GUI::Overlay
{
    ...
    bool Initialize(HWND hwnd, ID3D12Device* device, UINT bufferCount, DXGI_FORMAT format, ID3D12DescriptorHeap* srvHeap)
    {
        ...
        // Register HotkeyManager as the first render callback so it always
        // runs before any feature overlay (ESP, Menu, Aimbot, etc.).
        Core::Scheduler::RegisterRenderCallback("Overlay", []()
        {
    ...
    // Called by D3D12Hook after OnRenderFrame() to finalize the ImGui frame
    // and retrieve the draw data for GPU submission.
    ImDrawData* EndFrame()
    {
        std::scoped_lock lock(g_ImGuiMutex);
